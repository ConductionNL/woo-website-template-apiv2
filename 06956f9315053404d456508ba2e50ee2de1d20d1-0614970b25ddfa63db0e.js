(self["webpackChunkwoo_website_template_apiv2"] = self["webpackChunkwoo_website_template_apiv2"] || []).push([[659],{
...
const parseDateSafe=date=>{// Format 4: full ISO datetime with time component — Safari handles these correctly
if(date.includes("T")){return new Date(date);}// Format 3: yyyy-mm-dd (ISO date-only — parse manually to avoid Safari's UTC midnight issue)
const yyyymmdd=date.match(/^(\d{4})-(\d{2})-(\d{2})/);if(yyyymmdd){return new Date(parseInt(yyyymmdd[1]),parseInt(yyyymmdd[2])-1,parseInt(yyyymmdd[3]));}// Formats 1 & 2: dd-mm-yyyy or mm-dd-yyyy — same pattern, disambiguated by value
const dmyyyy=date.match(/^(\d{2})-(\d{2})-(\d{4})/);if(dmyyyy){const[,part1,part2,year]=dmyyyy.map(Number);// If part1 > 12 it can only be a day (dd-mm-yyyy); if part2 > 12 it can only be a day (mm-dd-yyyy)
const[day,month]=part1>12||part2<=12?[part1,part2]:[part2,part1];return new Date(year,month-1,day);}return new Date(date);};const translateDate=(language,date)=>{const parsedDate=parseDateSafe(date);switch(language){case"nl":i18n.dayNames=days_nl;i18n.monthNames=nl;return dateFormat(parsedDate,window.sessionStorage.getItem("DATE_FULL_MONTH")==="true"?"dd mmmm yyyy":"dd-mm-yyyy");case"en":i18n.dayNames=days_en;i18n.monthNames=en;return dateFormat(parsedDate,window.sessionStorage.getItem("DATE_FULL_MONTH")==="true"?"mmmm dd yyyy":"mm-dd-yyyy");}return dateFormat(parsedDate,"dd-mm-yyyy");// required default return due to i18n.language being basic typed
};
...
/* harmony export */ });
const removeHTMLFromString=html=>{// DOMParser creates an inert document: nothing is loaded or executed while parsing.
const doc=new DOMParser().parseFromString(html,"text/html");return doc.body.textContent||"";};
...
//# sourceMappingURL=06956f9315053404d456508ba2e50ee2de1d20d1-0614970b25ddfa63db0e.js.map