...
| NL_DESIGN_THEME_CLASSNAME | Ja                    | De naam van het thema van de organisatie                                                                           | string, max 255 characters                           | "openwoo-theme"                                                                               |
...
| JUMBOTRON_TITLE           | Nee                   | De titel in de jumbotron-kaart; `{ORGANISATION_NAME}` wordt vervangen door de organisatienaam, bij leeg wordt de standaardtekst met de organisatienaam getoond. NB geconfigureerde teksten worden niet vertaald door de taalswitch (WCAG 3.1.2) — combineer zo nodig met HIDE_LANGUAGE_SWITCH | string, max 255 characters                           | "Woo-publicaties van {ORGANISATION_NAME}"                                                        |
| JUMBOTRON_SUBTITLE        | Nee                   | De ondertitel in de jumbotron-kaart; `{ORGANISATION_NAME}` wordt vervangen zoals bij JUMBOTRON_TITLE, bij leeg wordt de standaardtekst met de organisatienaam getoond. NB wordt niet vertaald door de taalswitch — zie JUMBOTRON_TITLE | string, max 255 characters                           | "Op deze pagina vindt u de Woo-publicaties van {ORGANISATION_NAME}"                              |
| HIDE_LANGUAGE_SWITCH      | Nee                   | Verbergt bij "true" de NL/EN-taalswitch. Niet-vertaalde (Nederlandse) API-content kan geen correct lang-attribuut per element krijgen; een volledig Nederlandse pagina voldoet daarmee aan WCAG 3.1.2 | string, "true" of leeg                               | "true"                                                                                           |
